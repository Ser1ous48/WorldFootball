package com.example.worldfootball.utils
...
class Utils {
    companion object {
        ...
        fun Fragment.unauthorized(navigatonHandlder : () -> Unit){
            ...
            builder.setPositiveButton(getString(R.string.Registar)) { _, _ ->
                val preferences = requireActivity().getSharedPreferences("pref", Context.MODE_PRIVATE)
                preferences.edit().putString("token", null).apply()
                navigatonHandlder()
            }
            builder.setTitle(getString(R.string.Sessaoexpirada))
            builder.setMessage(getString((R.string.Sessaoexpirada_mensagem)))
            builder.create().show()
        }
